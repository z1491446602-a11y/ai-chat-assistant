...
- 单枚新兑换码最多为 1,000,000 积分；余额加法、持久化恢复和活动预留总额都受安全整数与一致性校验保护。历史超限码仅保留审计，不能兑换也不占用新码容量。
...
生产升级前的主数据约 16.7 MB，其中约 16.4 MB 是 225 个 AI owner 的历史；旧好友、公告和旧账号键合计约 156 KB。为避免未确认引用的历史媒体或数据丢失，本轮先完整备份并保留这些遗留键，运行路径不再使用它们。数据文件、备份和目录权限分别收紧为 0600、0600 和 0700。
...
部署前检查发现旧图片/Grok 上游仍使用公网 HTTP。已验证同一服务存在可用 HTTPS 端点和所需模型；上线已只替换 URL，不更换模型或密钥，也不调用付费生成做冒烟。
...
- `npm test`：54 个测试文件，817/817 通过；`npm run lint` 和 `npm run build` 退出 0。
...
- Gitee 已推送 `39cefbb`（积分安全边界）和 `e33fec1`（CORS 拒绝固定为中文 403）；最终 release 为 `auth-points-cors-20260713-2100-e33fec1`。发布包 308 项、2,994,464 bytes，SHA-256 为 `995ba482eaa6cbc5418b5bd12786e65361747ae4f5f91e302e8e1af0afc72a8f`，不含环境文件、运行数据或媒体。
- 已备份旧 release 指针、共享环境、根数据、主数据和 `.bak`；新 release 使用共享 `.env`、`storage` 和遗留根数据软链接。数据副本预检在备用端口通过健康与恶意跨域 403 检查后，才原子切换软链接。
- 公网健康、游客状态、游客普通会话、游客图片/视频 401、同源 CORS、管理员登录及 Secure/HttpOnly/SameSite Cookie、0.1 积分兑换、重复兑换 409、管理员重置密码与旧会话失效均已通过；没有调用付费图片或视频上游。
- `hello-kitty-chat.service` 为 active，部署后 RSS 为 187,596,800 bytes；共享目录为 0700，环境、主数据和 `.bak` 为 0600。Nginx 配置检查成功，部署时段没有 5xx、启动迁移错误或未处理异常；唯一记录到的 403 是验收故意发出的跨域拒绝。
- 迁移后的主数据有 2 个认证账号、1 枚已使用兑换码、1 条积分流水、0 条积分预留和 226 个 AI 历史 owner。遗留社交键和历史媒体未删除，避免在未逐项确认引用前损伤已有内容。
