...
媒体任务提交时先冻结积分，成功后扣除，失败、取消、排队拒绝或无法恢复时释放。提示词中写“生成 2 张”至“生成 5 张”时，图片任务会向上游并发发起对应数量的单张请求，最多 5 张；只按实际成功的请求张数扣积分，部分失败会保留成功图片并释放失败部分的冻结积分。积分使用整数单位保存，`1 unit = 0.1 积分`，避免浮点误差。
