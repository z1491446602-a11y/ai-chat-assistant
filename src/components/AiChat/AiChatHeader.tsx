...
export function AiChatHeader({
  ...
  return (
    <header className="relative flex min-h-[3.5rem] shrink-0 items-center justify-center border-b border-slate-100/80 bg-white/70 backdrop-blur-md px-20 pb-2 pt-[calc(env(safe-area-inset-top)+0.5rem)] text-center sm:py-2">
      <div className="min-w-0">
        <h1
          className="truncate text-[15px] font-bold leading-5 text-slate-800 tracking-tight"
          style={{ fontFamily: '"Microsoft YaHei UI", "PingFang SC", "Noto Sans SC", system-ui, sans-serif' }}
        >
          人工智障
        </h1>
        <p className="mt-0.5 truncate text-[11px] leading-4 text-slate-400 font-medium">内容由 AI 生成</p>
      </div>
      {!sidebarOpen && <button
        type="button"
        ...
        className="absolute right-3 top-[calc(env(safe-area-inset-top)+0.35rem)] inline-flex h-9 min-w-9 max-w-[11rem] items-center justify-center gap-2 rounded-full border border-slate-200/70 bg-white/80 px-2.5 text-xs font-medium text-slate-700 shadow-[0_4px_14px_rgba(15,23,42,0.06)] transition-colors hover:border-sky-300 hover:bg-sky-50 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 disabled:cursor-wait disabled:text-slate-400"
      >
        {authStatus === 'loading' ? (
