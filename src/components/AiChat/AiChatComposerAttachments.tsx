    ...
    <div className="mt-2 flex min-w-0 flex-1 items-center gap-1.5 overflow-visible">
      <div className="relative z-[70]">
        ...
        {props.showMoreActions ? (
          <div className="absolute bottom-12 left-0 z-[90] min-w-[148px] space-y-1 rounded-2xl border border-sky-100 bg-white/96 p-2 shadow-[0_14px_32px_rgba(15,23,42,0.12)] backdrop-blur">
      ...
      <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-visible">
        <div className="relative z-[80] flex min-w-0 shrink overflow-visible">
          ...
          {props.showImageProviderMenu ? (
            <div className="absolute bottom-12 left-0 z-[100] min-w-[176px] rounded-2xl border border-sky-100 bg-white/96 p-2 shadow-[0_18px_42px_rgba(15,23,42,0.16)]">
              {props.imageProviderOptions.map((option) => (
                <button type="button" key={option.value} onClick={() => props.onSelectImageProvider(option.value)} className="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm text-slate-700 hover:bg-sky-50">
